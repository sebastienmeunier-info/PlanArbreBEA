...
<link rel="stylesheet" href="/public/css/app.css"><title>Utilisateurs · <?= htmlspecialchars($application['name']) ?></title>
<body class="outdoor-mode">
<?php require __DIR__ . '/../partials/header.php'; ?>
<main class="auth-page admin-page">
  <h1>Utilisateurs</h1><p><a href="/admin/propositions">Gérer les propositions</a></p>
  ...
  <?php foreach ($users as $listedUser): ?><tr>
    ...
    <?php elseif ($user['role'] === 'administrateur' && $listedUser['role'] === 'contributeur'): ?>
      <form method="post" action="/admin/utilisateurs/role"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrfToken) ?>"><input type="hidden" name="user_id" value="<?= htmlspecialchars($listedUser['id']) ?>"><input type="hidden" name="role" value="administrateur"><button>Passer administrateur</button></form>
