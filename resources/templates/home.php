...
<main class="page-layout">
    <section class="intro"><h1><?= htmlspecialchars($pageTitle, ENT_QUOTES, 'UTF-8') ?></h1><p id="objective-statistics" data-proposed="<?= (int) $statistics['proposed'] ?>" data-validated="<?= (int) $statistics['validated'] ?>" data-planted="<?= (int) $statistics['planted'] ?>">Objectif : <?= number_format((int) $planting['target_count'], 0, ',', ' ') ?> <?= $treeProposal ? 'arbres' : 'plantations' ?> — <?= (int) $statistics['proposed'] ?> <?= $treeProposal ? 'arbres proposés' : 'plantations proposées' ?>, <?= (int) $statistics['validated'] ?> <?= $treeProposal ? 'arbres validés' : 'plantations validées' ?>, <?= (int) $statistics['planted'] ?> arbres plantés.</p></section>
    ...
    <section class="form-panel">
        <form id="proposal-form" enctype="multipart/form-data" novalidate>
            ...
            <label class="privacy-consent"><input name="notification_consent" type="checkbox" value="1"<?= !empty($user['notification_consent_at']) ? ' checked' : '' ?>> J’accepte de recevoir par e-mail les notifications liées à cette proposition.</label>
            <?php if ($treeProposal): ?>
...
<script>window.PlantonsConfig = <?= json_encode(['center' => $territory['center'], 'zoom' => $map['default_zoom'], 'territoryUrl' => !empty($dataSources['territory']['file']) ? $routeUrl((string) ($dataSources['territory']['url'] ?? '')) : '', 'sectorsUrl' => !empty($dataSources['sectors']['file']) ? $routeUrl((string) ($dataSources['sectors']['url'] ?? '')) : '', 'sectorType' => $territory['type_secteur'], 'proposalsUrl' => $routeUrl($activeDataSource['url']), 'proposalUrl' => $submissionUrl, 'geocodingSearchUrl' => $geocoding['search_url'], 'geocodingReverseUrl' => $geocoding['reverse_url'], 'markerShape' => $treeProposal ? 'cross' : 'round', 'treeProposal' => $treeProposal, 'targetCount' => (int) $planting['target_count'], 'maxPhotos' => $security['max_photos_per_proposal'], 'maxObjectives' => $planting['max_objectives_per_proposal'], 'objectives' => $planting['objectives'], 'conditionings' => $planting['tree_conditioning'], 'treeSizes' => $planting['tree_sizes']], JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>;</script>
