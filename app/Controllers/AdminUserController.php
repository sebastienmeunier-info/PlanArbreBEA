...
final class AdminUserController {
    ...
    public function index(Request $request): never { $auth = $this->auth(); $this->guard($auth); Response::html($this->app->view()->render('admin/users', ['users' => $this->repository()->all(), 'csrfToken' => $auth->csrfToken(), 'user' => $auth->currentUser(), 'application' => $this->app->config('app')])); }
